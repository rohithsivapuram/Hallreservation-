...
def makeBooking():
    print()
    for Num,Name,Price in halllist: 
        print(Num,Name,"=",Price)
    print()
    
    validhall = False
    while validhall == False:
        try:
            hallchoice = input("Which hall would you like to book? Please choose a number from the list above: ")
            if hallchoice == "quit" or hallchoice == "Quit" or hallchoice == "QUIT":
                return
            elif int(hallchoice) < 1 or int(hallchoice) > 27:
                 print("This number is not on the list.")
            else:
                hall = halllist[int(hallchoice) - 1]
                validhall = True
        except:
    ...
    booking = (name , str(hall[1]) , str(weekday[1]) , str(date[1]) , str(time[1]) , email)
...
while 1==1:
    print()
    print("Welcome to Newyork hall booking system! Here you can choose a hall, then select a date and time. Say 'quit' at any time to exit the process.")
    print()
